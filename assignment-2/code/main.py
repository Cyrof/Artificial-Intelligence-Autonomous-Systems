...
def nb(args):
  ...
  if args.mode == "train":
    ...
    print(f"The predicted values for validation: {nb.predict(x_val)[0]}")
    print(f"The predicted values for test: {nb.predict(x_test)[0]}")
  else: 
...
def compare_models(args):
  ...
  nb_pred, nb_score = nb.predict(x_test)
  plot_confusion_matrix(nb_conf, 'Naive Bayes')
  plot_roc_curve(y_test, nb_score, "Naive Bayes", len(nb.class_probs))
  ...
  cnn_pred, cnn_score = cnn.predict(test_dataloader)
